...
<!--            <el-input></el-input>-->
            <div v-for="friend in friends"
                 ...
                 @click="this.curFriend = friend;loadMessages(this.curFriend.id)"
                 class="friend">
...
<!--            <el-input></el-input>-->
            <div v-for="request in requests"
              ...
              <div style="flex: 2;">
                <el-button @click="agree(request.messageId,request.user.id)" type="success">同意</el-button>

              </div>
              <div style="flex: 3;">
                <el-button @click="refuse(request.messageId,request.user.id)" type="danger">拒绝</el-button>
          ...
          <el-collapse-item title="添加好友" name="3">
            <!--遍历非好友-->
<!--            <el-input></el-input>-->
            <div v-for="nonFriend in nonFriends"
                 style="height: 80px;display: flex;cursor:pointer"
                 @click=""
                 class="friend">
        ...
        <el-form :inline="true" label-suffix="：" @submit.native.prevent>
          <el-form-item style="width: 100%">
            <el-input
                ...
                show-word-limit
                maxlength="30"
                @keydown.enter.native="send(curFriend.id)"
                clearable
            />
          </el-form-item>
        </el-form>
  ...
  methods: {
    ...
    refuse(messageId,toId){
      delay(() => {
        ...
        if (user == null || window.webSocket == null) {
          ...
          window.webSocket.send(JSON.stringify(new Message(null, 9, new MessageContent(user.id, toId, null), messageId)));
    ...
    agree(messageId,toId){
      delay(() => {
        ...
        if (user == null || window.webSocket == null) {
          ...
          window.webSocket.send(JSON.stringify(new Message(null, 8, new MessageContent(user.id, toId, null), messageId)));
    ...
    addFriend(nonFriendId) {
      delay(() => {
        ...
        if (user == null || window.webSocket == null) {
          ...
          window.webSocket.send(JSON.stringify(new Message(null, 7, new MessageContent(user.id, nonFriendId, null), null)));
        }
      }, 50)
    ...
    send(toUserId) {
      if (window.webSocket != null && window.webSocket.readyState == 1) {
        ...
        window.webSocket.send(JSON.stringify(new Message(null, 1, new MessageContent(user.id, toUserId, this.sendMessage), null)));
      } else {
        ElNotification.error({title: "error", message: '请先登陆', offset: 50});
      }
